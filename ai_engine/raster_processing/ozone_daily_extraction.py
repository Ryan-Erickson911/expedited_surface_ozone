...
from rasterio import transform as tf
...
def add_buffered_features(points_df, cols, path, buff=100):
    ...
    for date_val, group in points_df.groupby('date'):
        ...
        with ro.open(full_path, driver='GTiff') as raster:
  ...
  for date_str, group in points_df.groupby('date'):
    ...
    if file:
      ...
      for i, col in enumerate(col_names, start=1):
        if col == 'SKIP':
          continue
        with ro.open(full_file_name) as dataset:
...
#Creates a full list of monitors and potential dates for which each monitor should have a values (i.e: 7 monitors, 4 years (assume tp=2013-2023) == 365*3+366 * 7 Mn = 10,017 days)
def make_full_monitors(df, y_st='2000', y_ed='2023', fpath=''):
    ...
    return ap_df
# Quick plot a nice raster
def show_raster(file_path, title='Raster Image'):
    with ro.open(file_path) as src:
...
###################################################################################################
# Main code
final_path = os.path.join(os.path.expanduser('~'), 'Documents','Github','UCBMasters','data','results','final_results','monitor_tables')    
...
elev_path = os.path.join(os.path.expanduser('~'), 'Documents','Github','UCBMasters','data','tifs','elevation','elevation.tif')
gridmet_path = os.path.join(os.path.expanduser('~'), 'Documents','Github','UCBMasters','data','tifs','gridmet','daily')
ozone10km_path = os.path.join(os.path.expanduser('~'), 'Documents','Github','UCBMasters','data','tifs','ozone10km','daily')
ndvi_path = os.path.join(os.path.expanduser('~'), 'Documents','Github','UCBMasters','data','tifs','ndvi','daily')
ntl_path = os.path.join(os.path.expanduser('~'), 'Documents','Github','UCBMasters','data','tifs','ntl','daily')
arsl_path = os.path.join(os.path.expanduser('~'), 'Documents','Github','UCBMasters','data','tifs','s5p','aerosols','daily')
no2_path = os.path.join(os.path.expanduser('~'), 'Documents','Github','UCBMasters','data','tifs','s5p','no2','daily')
hcho_path = os.path.join(os.path.expanduser('~'), 'Documents','Github','UCBMasters','data','tifs','s5p','hcho','daily')
s5p_path = os.path.join(os.path.expanduser('~'), 'Documents','Github','UCBMasters','data','tifs','s5p','o_3_1km','daily')
clds_path = os.path.join(os.path.expanduser('~'), 'Documents','Github','UCBMasters','data','tifs','s5p','clouds','daily')
co_path = os.path.join(os.path.expanduser('~'), 'Documents','Github','UCBMasters','data','tifs','s5p','co','daily')
# Make sure to get col lists from gee_1980_2024.py'SKIP'
...
with ro.open(elev_path) as raster:
...
for col, path in buffer_files: 
    gdf_2018_2024_26949 = add_buffered_features(gdf_2018_2024_26949,col,path, buff=500)
    for i in col:
        if i == 'SKIP':
            continue
        else:
...
# gdf_2018_2024_26949 = gdf_2018_2024_26949.drop(columns=['SKIP']) 
   
buffered_mode_values = gdf_2018_2024_26949[['site_id','date']+buffer_cols]
gdf_2018_2024_full = pd.merge(gdf_2018_2024, buffered_mode_values, how='left',on=['site_id','date']) # type: ignore
count_NAs(gdf_2018_2024_full.columns,gdf_2018_2024_full)
gdf_2018_2024_full.to_csv(os.path.join(os.path.expanduser('~'),'Documents','Github','UCBMasters','data','tables','ozone_daily_2018_2023_data.csv'))
