...
def get_daily_epa_data(year_start=2018, year_end=2024, param_code="44201",state_res="04", county_res=["Maricopa", "Pinal", "Pima"]):
    ...
    email="ryanerickson407@gmail.com" #email for login (EPA)
    key="taupecrane93" #EPA Key
    ...
    for year in years_list:
        ...
        download=get_epa_data(email=email, key=key, param=param, state_code=state, year=str(year))
        if download.empty:
    ...
    return monitors
# Main function for final monthly monitor data. Gathers data from http server on EPA website. An account must be created and verified by the EPA to gain access. 
def get_monthly_epa_data(year_start, year_end, param_code="44201",state_res="04", county_res=["Maricopa", "Pinal", "Pima"]):
    years_list=range(year_start, year_end + 1) #list of years
    email="ryanerickson407@gmail.com" #email for login (EPA)
    key="taupecrane93" #EPA Key
    param=param_code #aerosol param code from EPA
    ...
    for year in years_list:
        print(f'\nDownloading {year} monitor data from EPA')
        months=pd.date_range(start=f'{year}-01', end=f'{year+1}-01', freq="MS").strftime('%Y-%m').tolist()
        download=get_epa_data(email=email, key=key, param=param, state_code=state, year=str(year))
        if download.empty:
            continue
        print(f'Adding {year} monthly averages to dataset')
        filtered_oz=download[download["county"].isin(photuc_metro_counties)]
        filtered_oz['site_id']=filtered_oz['state_code'] + filtered_oz['county_code'] + filtered_oz['site_number']
        monthly_avg=(filtered_oz.groupby(['site_id', pd.to_datetime(filtered_oz['date_local']).dt.to_period('M').astype(str)]).agg(avg_max_val=('first_max_value', 'mean'),latitude=('latitude', 'first'),longitude=('longitude', 'first'),state_code=('state_code', 'first'),county_code=('county_code', 'first'),datum=('datum', 'first')).reset_index().copy())
        site_ids=monthly_avg['site_id'].unique().copy()
        all_dates=pd.DataFrame([(site_id, date) for site_id in site_ids for date in months], columns=['site_id', 'date_local'])
        final=all_dates.merge(monthly_avg, on=['site_id', 'date_local'], how='left')
        final[['latitude', 'longitude', 'state_code', 'county_code', 'datum']]=(final.groupby('site_id')[['latitude', 'longitude', 'state_code', 'county_code', 'datum']].ffill().bfill())
        all_data.append(final)
    final_df=pd.concat(all_data, ignore_index=True)
    monitors=final_df[['site_id', 'avg_max_val', 'date_local', 'latitude', 'longitude', 'datum']]
    monitors.columns=['site_id', 'avg_monthly_val', 'date', 'lat', 'long', 'datum']
    print(f'Number of unique monitors: {monitors["lat"].nunique()}')
    print('Complete')
    return monitors
#Path to final data. This is where data has been fully cleaned, processed, and is ready to be used in the model.
final_path = os.path.join(os.getcwd(), "data",'monitor_tables')
oz_dailydf_2018_2024=get_daily_epa_data(2023, 2024, param_code='44201') #ozone 
# oz_monthlydf_2018_2024=get_monthly_epa_data(2018, 2024, param_code='44201') #ozone 
# no2_dailydf_2018_2024=get_daily_epa_data(2018, 2024, param_code='42602') #no2 
# no2_monthlydf_2018_2024=get_monthly_epa_data(2018, 2024, param_code='42602') #no2
# Exporting to CSV
if not os.path.exists(final_path): os.makedirs(final_path)

# return the list of monitoring stations

oz_dailydf_2018_2024.to_csv(final_path+'\\oz_dailydf_2018_2024.csv', sep=";", index=True) #65 unique monitor ids
# oz_monthlydf_2018_2024.to_csv(final_path+'\\oz_monthlydf_2018_2024.csv', sep=";", index=True)

# no2_dailydf_2018_2024.to_csv(final_path+'\\no2_dailydf_2018_2024.csv', sep=";", index=True)
# no2_monthlydf_2018_2024.to_csv(final_path+'\\no2_monthlydf_2018_2024.csv', sep=";", index=True)

### Making GDF -> Might not need
# oz_gdf = mon_to_gdf(oz_df_2018_2024)
# no2_gdf = mon_to_gdf(no2_df_2018_2024)