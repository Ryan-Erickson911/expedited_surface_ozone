...
def get_epa_data(param, state_code, sdate, edate, email=epa_email, key=epa_key):
    stime = datetime.date(sdate)
    etime = datetime.date(edate)
    s = stime.strftime("%Y%m%d")
    e = etime.strftime("%Y%m%d")
    url=f"https://aqs.epa.gov/data/api/dailyData/byState?email={email}&key={key}&param={param}&bdate={s}&edate={e}&state={state_code}"
    try:
...
def get_monitor_data(param="44201",state="04", start=2018, end=2024):
    years_list=range(year_start, year_end + 1)
    #county names (list) photuc_metro_counties=county_res
    all_data=[]
    quick_summary=[]
    for year in years_list:
        print(f'\nDownloading {year} monitor data from EPA')
        download=get_epa_data(param=param, state_code=state, )
        if download.empty:
